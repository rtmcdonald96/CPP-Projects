  ...
  std::unique_ptr<SessionHandler> SessionHandler::createSession( const UserCredentials & credentials )
  {
    ...
    try
    {
        ...
        )
      {
        ...
        if( credentials.roles[0] == "Manager" ) return std::make_unique<Domain::Session::ManagerSession>( credentials );
        if( credentials.roles[0] == "Sales Rep" ) return std::make_unique<Domain::Session::SalesRepSession>( credentials );
        if( credentials.roles[0] == "System Admin" ) return std::make_unique<Domain::Session::SystemAdminSession>( credentials );
